		...
		<div class="flex items-center gap-2">
			{#if modelFilterLabel}
				<span class="text-blue-500 font-normal"
					>Filtered model: <span class="font-medium">{modelFilterLabel}</span></span
				>
			{/if}
			{#if userFilterLabel}
				<span class="text-blue-500 font-normal ml-2"
					>Filtered user: <span class="font-medium">{userFilterLabel}</span></span
				>
			{/if}
			{#if selectedPair}
				<span class="text-blue-500 font-normal ml-2"
			...
			<select
				...
				on:change={(e) =>
					onModelModeChange(
						(e.currentTarget as HTMLSelectElement).value as 'or' | 'and' | 'selected' | 'requested'
					)}
			>
		...
		<div class="grid md:grid-cols-2 gap-4">
					...
					<tbody>
						{#each pairs as pair, idx (`${pair.requested_model_id}-${pair.selected_model_id}`)}
							<tr
								...
								class:bg-blue-50={selectedPair?.requested_model_id === pair.requested_model_id &&
									selectedPair?.selected_model_id === pair.selected_model_id}
								class:dark:bg-blue-950={selectedPair?.requested_model_id ===
									pair.requested_model_id &&
									selectedPair?.selected_model_id === pair.selected_model_id}
						...
						{#if pairs.length === 0}
							<tr
								><td colspan="5" class="px-3 py-2 text-center text-gray-400">No routing data</td
								></tr
							>
						{/if}
					</tbody>
			...
			<div>
				...
				>
					...
					{#if selectedPair}
						<button class="text-blue-500 hover:text-blue-600" on:click={onClearPair}
							>Clear pair</button
						>
					{/if}
				</div>
							...
							{#each events as event (event.message_id)}
								<tr class="bg-white dark:bg-gray-900 dark:border-gray-850 text-xs">
									<td class="px-3 py-1 text-gray-400"
										>{new Date(event.created_at * 1000).toLocaleString()}</td
									>
